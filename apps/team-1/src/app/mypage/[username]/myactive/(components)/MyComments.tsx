    ...
    <div className="flex flex-col gap-[70px]">
        ...
        <div className="fixed top-0 left-0 w-full h-full bg-[#000000] bg-opacity-40 flex items-center justify-center">
          <div className="fixed top-1/2 left-1/2 w-[331.5px] h-[167.41px] bg-neutral-0 rounded-[10px] flex flex-col justify-between">
            <div className="p-[30.75px] flex flex-col gap-[18px]">
              ...
              <p className="body-7 text-neutral-100 text-center">이 댓글을 삭제하시겠습니까?</p>
            </div>
      ...
      <div className="flex flex-col gap-[40px]">
        {data && data?.items.map(item => (
          <div key={item.id} className="w-[656px] h-[152px] border-b border-stroke-10 flex flex-col">
            <div className="px-[15px] py-[10px] flex flex-col flex-grow gap-[10px]">
              <div className="flex justify-between gap-[5px]">
                <p className="body-5 text-neutral-100 line-clamp-2">{item.content}</p>
                {isDelete && (
                  <button
                    key={item.id}
              ...
              <p className="flex items-center body-5 text-neutral-50">{formatDate(item.updated)}</p>
            </div>
            <div className="flex items-center px-[16px] py-[10px] justify-between">
              <span className="body-7-bold text-neutral-50">{item.expand?.logId.title}에 남긴 댓글</span>
              <button className="body-7-bold text-neutral-50">{item.replies.length}개의 답글</button>
            </div>
          </div>
        ))}
      </div>
