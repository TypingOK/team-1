...
const MyactiveTap = ({ params, searchParams }: mypageProps) => {
  return (
    <div className="flex flex-col gap-[20px] p-[50px]">
      ...
      <TabRoot value={searchParams?.target ? searchParams.target : "interest"}>
        <TabList className="w-[380px] flex justify-around">
          <Link href={`/mypage/${params.username}/myactive?target=interest`}>
            <TabTrigger value="interest" className="text-sm text-neutral-90">
              관심 로그
            </TabTrigger>
          </Link>
          <Link href={`/mypage/${params.username}/myactive?target=recent`}>
            <TabTrigger value="recent" className="text-sm text-neutral-90">
              최근 본 로그
            </TabTrigger>
          </Link>
          <Link href={`/mypage/${params.username}/myactive?target=comment`}>
