...
import { useGetTargetLikeLogs } from "@/hooks/queries/useGetMyLikeLogs";
...
import useDeleteLikesLogs from "@/hooks/queries/useDeleteLikesLogs";

const MyLikesLog = () => {
  const userData = pb.authStore.model as userTypes;
  const searchParams = useSearchParams();
  const page = Number(searchParams.get("page")) || 1;
  const { data } = useGetTargetLikeLogs(page, userData?.id, page, 30);
  ...
  const handleSelectAll = () => {
    ...
    if (!isAllSelect) {
      const allItemIds =
        data?.items.flatMap(item => item.expand?.logId.map(log => log.id)) ||
        [];
      setSelectedItems(
        isAllSelect
          ? []
          : (allItemIds.filter(id => id !== undefined) as string[]),
      );
    } else {
      setSelectedItems([]);
    }
    ...
    <div className="flex flex-col gap-[70px]">
      ...
      {data &&
        data.items.map(item => (
          <div
            key={item.id}
            className="grid grid-cols-2 gap-x-[52px] gap-y-[30px]"
          >
              ...
              <div key={log.id} className="flex flex-col gap-[10px]">
                ...
                <div className="flex gap-[5px] items-center">
                  <div className="w-[24px] h-[24px]">
                    <img
                      src={`https://nf01uyzvha.execute-api.ap-northeast-2.amazonaws.com/api/files/_pb_users_auth_/${log.expand?.userId.id}/${log.expand?.userId.profileImage}`}
                    ></img>
                  </div>
                  <span className="body-7 text-neutral-30">
                    {log.expand?.userId.username}
                  </span>
                </div>
              </div>
            ))}
