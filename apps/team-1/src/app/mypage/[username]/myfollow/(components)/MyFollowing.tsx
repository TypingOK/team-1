    ...
    <div>
      {data && data?.items.length !== 0
            ...
            {data && data.items.map(item => (
              <div key={item.id} className="flex justify-between items-center">
                <div className="flex gap-[15px]">
                  ...
                  />
                  <div>
                    <p className="body-4-bold">{item.expand?.followingId.username}</p>
                    <p className="flex gap-[5px] text-[14px] font-semibold text-neutral-50">로그 전체보기
        ...
        : <div className="flex flex-col item-center justify-center pt-[232px] gap-[18px]">
          ...
          <p className="text-center body-5-bold">내가 팔로잉하는 사용자가 없습니다.</p>
        </div>
      }
    </div>
  )
