...
const MyArchive = () => {
  ...
  const deleteCommentsMutation = useDeleteMyAchive(selectedItems);
      ...
      {data && data?.items.length != 0 ? <div className="flex justify-between">
        ...
        {isDelete ? (
          ...
          <Button
            ...
            className="w-[165px] h-[40px] text-[16px] font-semibold"
          >
            아카이브 편집
          </Button>
        )}
        ...
        : <div className="flex flex-col item-center justify-center pt-[232px] gap-[18px]">
          ...
          <p className="text-center body-5-bold">작성한 아카이브가 없습니다</p>
        </div>
      }
      {isModalOpen && (
        <div className="fixed top-0 left-0 w-full h-full bg-[#000000] bg-opacity-40 flex items-center justify-center z-10">
          <div className="fixed top-1/2 left-1/2 w-[331.5px] h-[167.41px] bg-neutral-0 rounded-[10px] flex flex-col justify-between">
            <div className="p-[30.75px] flex flex-col gap-[18px]">
              ...
              <p className="body-7 text-neutral-100 text-center">이 아카이브를 삭제하시겠습니까?</p>
            </div>
        ...
        {data && data.items.map(item => (
          <div key={item.id} className="flex flex-col gap-[10px]">
            <div className="relative">
              <img className="w-[302px] h-[158px] rounded-[10px] object-contain" src={item.thumbnail} alt="썸네일 없음"></img>
              {isDelete && (
                <button
                  key={item.id}
            ...
            <p className="body-6-bold text-neutral-90">{item.archiveName}</p>
            <div className="flex gap-[7px]">
              <span className="body-7 text-neutral-50">{formatDate(item.created)}</span>
              <span className="body-7 text-stroke-10">|</span>
              <img src="/icons/mypage/file.svg" />
              <span className="body-7 text-neutral-50">3</span>
            </div>
          </div>
        ))}
      </div>
